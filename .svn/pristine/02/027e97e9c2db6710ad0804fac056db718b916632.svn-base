$.fn.fishBone = function(data) {
    ...
    function creataFishBone(data) {
        ...
        var bd = $("<div class='bd' id='haha'></div>");
        var ul_item = $("<ul/>");
        //遍历数据
        $(data).each(function(index) {
            ...
            if(itemclass=='top'){
                $.each(this, function(name, value) {
                    if (name == '时间') {
                        var li = $("<li class='line-first' id='li" + index +"'>" + value + "</li>")
                ...
                $.each(this, function(name, value) {
                    if (name == '事件') {
                        var li = $("<li class='title'  id='li" + index +"'></li>");
            ...
            $.each(this, function(name, value) {
                if (name != '事件' && name != '时间') {
                    var li = $("<li  id='li" + index +"'>" + name + "：" + value + "</li>").css("border-left","1px solid "+color);
                    li.appendTo(ul);
                }
            });
          //封装审理时间和案号
