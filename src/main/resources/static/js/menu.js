...
layui.define('jquery', function(exports) {
	...
	var menu = {
			...
			$.getJSON(curl, function(res) {
				...
				$.each(menuList, function(index, val) {
					...
					if (val.children && val.children.length) {
						//有二级子菜单
						setTimeout(function() {
							that.getSubMenu(val.id, val.children, false);
						}, 500);
					}
				});
				$('#side-nav').empty();
		 ...
		 * @param {Boolean} isSpace  子菜单是否空格
		 */
		getSubMenu: function(id, subList, isSpace) {
			var that = this;
			var subItems = [];
			...
			$.each(subList, function(idx, sub) {
				...
				if (sub.children && sub.children.length) {
					var subItem = '<li id="menu' + sub.id + '"><a _href="' + sub.url + '"><i class="iconfont nav_right">&#xe697;</i><cite>' +
					sub.name + '</cite></a></li>';
				    subItems.push(subItem);
					setTimeout(function() {
						that.getSubMenu(sub.id, sub.children, true);
					}, 1500);
				}else{					
					if(isSpace){
						var subItem = '<li class="fourli" id="menu' + sub.id + '"><a _href="' + sub.url + '"><cite>' +
						sub.name + '</cite></a></li>';
					}else{
