...
layui.define(['jquery', 'form', 'layer', 'element'], function(exports) {
	...
	$('.container').on('click', '.left_open i', function(event) {
		if($('.left-nav').css('left') == '0px') {
			...
			$('.container .left_open i').removeClass('layui-icon-shrink-right').addClass('layui-icon-spread-left');
		} else {
			//此处左侧菜单是隐藏状态，点击显示
			$('.left-nav').animate({
				left: '0px'
			...
			$('.container .left_open i').removeClass('layui-icon-spread-left').addClass('layui-icon-shrink-right');
		}
	});
	//点击遮罩背景，左侧菜单隐藏
    ...
    // 添加新窗口
	/*$("body").on("click",".right li dl dd a",function(){
		var url = $(this).attr('data-url');
		var title = $(this).find('cite').html();
		var index = $('.right li dl dd a').index($(this));
