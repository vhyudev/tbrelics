...
function showResources(thisData) {
    ...
    layui.each(thisData, function(index, item) {
        ...
        /*arr.push('<div class="relic layui-row">'
            + '<div class="relic-data">'
            + '<h2>' + relicName + '</h2>'
            + '<input type="hidden" class="relicId" value="'+id +'"><div style="margin-top: 7px">'
            ...
            + age + '</button>'
            + '<button type="button" class="date layui-btn layui-btn-sm layui-btn-radius layui-btn-primary">'
            + date + '</button>'
            + '<button type="button" class="fcrclass layui-btn layui-btn-sm layui-btn-radius layui-btn-primary">'
            + fcrclass + '</button>'
            +'</div></div><div class="relic-img">'
        );*/

        var Img = item.resourceList;
        if(Img.length>5){
            for(var k=0;k<5;k++){
                var imgId = Img[k].id;
            ...
            var imgsStr = JSON.stringify(Img).replace(/"/g, '&quot;');
